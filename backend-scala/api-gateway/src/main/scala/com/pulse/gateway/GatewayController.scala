...
class GatewayController(
  ...
  get("/api/v1/health") { _: Request =>
    Future.value(response.ok.json(Map("status" -> "ok", "service" -> "pulse-gateway")))
  }
...
class PulseGatewayServer(config: ServiceConfig) extends com.twitter.server.TwitterServer:
  def main(): Unit =
